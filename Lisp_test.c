...
int main(int argc, char** argv) {
  ...
  Object* tclam3_str = QSTRING_NEW1(
    "(lambda (x) "
    "(if (>= x 0) "
    "  x"
    "  (* x -1)"
  ...
  Object* dlam = Object_accept(Lisp_eval_string(dlam_str));
  ObjectUtil_eprintf("tclam4 = %v\n", dlam);
  nassert(Object_is_null(dlam));
  Object* dlam_res1 = Object_accept(Lisp_eval_string(QSTRING("(abs -1)")));
  nassert(Object_type(dlam_res1) == SYMBOL_NUMBER);
  nassert(Object_cmp(dlam_res1, QNUMBER(1)) == 0);
  ObjectUtil_eprintf("dlam_res1 = %v\n", dlam_res1);
